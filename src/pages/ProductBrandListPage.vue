<script setup>
  ...
  import { ref } from 'vue'
  import { createProductBrand, deleteProductBrand, getProductBrand, sortProductBrand, updateProductBrand } from '@/api/productBrand'
  import DeleteDialog from '@/components/MainDeleteDialog.vue'
  import MainDragList from '@/components/MainDragList.vue'
  import EditDialog from '@/components/ProductBrands/EditDialog.vue'
  ...
  function loadItems ({ page, itemsPerPage }) {
    if (loading.value) return
    loading.value = true
    getProductBrand({ page, itemsPerPage, keyword: keyword.value }).then(({ items, total }) => {
  ...
  function sendSortOrderRequest () {
    ...
    loading.value = true
    sortProductBrand(updatedSortData)
      .then(response => {
  ...
  function sendCreateRequest (data) {
    ...
    createProductBrand(data)
      .then(response => {
        console.log(response)
        appStore.setSnackbar({ show: true, message: '成功新增商品品牌', type: 'success' })
        editDialogRef.value.closeDialog()
      })
      .catch(error => {
        appStore.setSnackbar({ show: true, message: error.message || '發生錯誤', type: 'error' })
        console.error('新增商品品牌失敗', error)
      })
      .finally(() => {
        loading.value = false
        loadItems({ page: page.value, itemsPerPage: itemsPerPage.value })
  ...
  function sendUpdateRequest (data) {
    if (loading.value) return
    loading.value = true
    updateProductBrand(data.id, data)
      .then(response => {
        console.log(response)
        appStore.setSnackbar({ show: true, message: '成功更新商品品牌', type: 'success' })
        editDialogRef.value.closeDialog()
      })
      .catch(error => {
        appStore.setSnackbar({ show: true, message: error.message || '發生錯誤', type: 'error' })
        console.error('更新商品品牌失敗', error)
      })
      .finally(() => {
        loading.value = false
        loadItems({ page: page.value, itemsPerPage: itemsPerPage.value })
  ...
  function sendDeleteRequest (id) {
    ...
    deleteProductBrand(id)
      .then(response => {
        console.log(response)
        appStore.setSnackbar({ show: true, message: '成功刪除商品品牌', type: 'success' })
      })
      .catch(error => {
        appStore.setSnackbar({ show: true, message: error.message || '發生錯誤', type: 'error' })
        console.error('刪除商品品牌失敗', error)
      })
      .finally(() => {
        loading.value = false
        loadItems({ page: page.value, itemsPerPage: itemsPerPage.value })
          ...
          <v-card-text v-show="!sortMode">
            <v-row class="w-full">
              ...
              <v-col class="d-flex justify-end" md="3">
                ...
                <v-btn color="lime" :prepend-icon="mdiPlus" @click="openEditDialog(null)">新增品牌</v-btn>
              </v-col>
            </v-row>
          </v-card-text>
